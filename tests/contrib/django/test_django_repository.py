...
def make_category(**kwargs) -> Category:  # type: ignore[type-arg]
    defaults: dict = dict(category_id=uuid4(), name="Test Category", external_id=None)
    defaults.update(kwargs)
    return Category(**defaults)


def make_item(**kwargs) -> Item:  # type: ignore[type-arg]
    defaults: dict = dict(item_id=uuid4(), external_id=None)
    defaults.update(kwargs)
...
def test_item_model_save_without_external_id() -> None:
    repo = make_repo()
    item = make_item(external_id=None)
    repo.save_item(item)
    result = repo.get_item(item.item_id)
    assert result is not None
...
# ---------------------------------------------------------------------------
# 032/041 — external_id field: unique (replaces db_index)
# ---------------------------------------------------------------------------
...
class TestExternalIdIndex:
    """Assert that external_id fields have unique constraint (spec 041: replaces db_index)."""

    def test_item_external_id_field_is_unique(self) -> None:
        assert ItemModel._meta.get_field("external_id").unique is True

    def test_category_external_id_field_is_unique(self) -> None:
