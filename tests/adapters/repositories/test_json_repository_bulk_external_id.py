"""Tests for JsonRepository bulk external-id lookup methods (spec 052)."""
...
def test_items_all_ids_found(repo: JsonRepository) -> None:
...
def test_items_some_ids_missing(repo: JsonRepository) -> None:
...
def test_items_all_ids_missing(repo: JsonRepository) -> None:
    result = repo.get_items_by_external_ids(["no-such", "also-missing"])
    assert result == {}


def test_items_duplicate_ids(repo: JsonRepository) -> None:
...
def test_items_blank_ids_ignored(repo: JsonRepository) -> None:
    item = Item(external_id="ext-real")
    repo.save_item(item)
    result = repo.get_items_by_external_ids(["ext-real", "", "   "])
    # Blank and whitespace IDs are pre-filtered by the service; adapter receives them too
    # but should not match since no item has "" or "   " as external_id
    assert "ext-real" in result
    assert "" not in result
    assert "   " not in result


def test_items_empty_input(repo: JsonRepository) -> None:
...
def test_items_enabled_true(repo: JsonRepository) -> None:
...
def test_items_enabled_false(repo: JsonRepository) -> None:
    enabled_item = Item(external_id="ext-enabled", enabled=True)
    disabled_item = Item(external_id="ext-disabled", enabled=False)
    repo.save_item(enabled_item)
    repo.save_item(disabled_item)
...
def test_items_enabled_none(repo: JsonRepository) -> None:
    enabled_item = Item(external_id="ext-enabled", enabled=True)
    disabled_item = Item(external_id="ext-disabled", enabled=False)
    repo.save_item(enabled_item)
    repo.save_item(disabled_item)
...
def test_categories_all_ids_found(repo: JsonRepository) -> None:
...
def test_categories_some_ids_missing(repo: JsonRepository) -> None:
...
def test_categories_all_ids_missing(repo: JsonRepository) -> None:
    result = repo.get_categories_by_external_ids(["no-such", "also-missing"])
    assert result == {}


def test_categories_duplicate_ids(repo: JsonRepository) -> None:
...
def test_categories_blank_ids_ignored(repo: JsonRepository) -> None:
...
def test_categories_empty_input(repo: JsonRepository) -> None:
    result = repo.get_categories_by_external_ids([])
    assert result == {}


def test_categories_enabled_true(repo: JsonRepository) -> None:
...
def test_categories_enabled_false(repo: JsonRepository) -> None:
    enabled_cat = Category(name="Enabled", external_id="cat-enabled", enabled=True)
    disabled_cat = Category(name="Disabled", external_id="cat-disabled", enabled=False)
    repo.save_category(enabled_cat)
    repo.save_category(disabled_cat)
...
def test_categories_enabled_none(repo: JsonRepository) -> None:
    enabled_cat = Category(name="Enabled", external_id="cat-enabled", enabled=True)
    disabled_cat = Category(name="Disabled", external_id="cat-disabled", enabled=False)
    repo.save_category(enabled_cat)
    repo.save_category(disabled_cat)
