...
- [ ] **FR-011** — `category list` prints all categories; exit 0 always
...
- [ ] **FR-014** — `item add --external-id X --name X [--description X] [--category-id UUID [--sort-index INT]] [--tag-id UUID]`; `--name` required at CLI layer
- [ ] **FR-015** — `item list` prints all items; exit 0 always
- [ ] **FR-016** — `item delete <id>` deletes and prints confirmation
- [ ] **FR-017** — `item update <id> [--name X] [--description X] [--enable|--disable] [--category-id UUID [--sort-index INT]] [--tag-id UUID]`; ≥1 option required
...
- [ ] **FR-026** — `Item.name: Annotated[str, Field(max_length=256)] = ""`
- [ ] **FR-027** — `Item.description: Annotated[str, Field(max_length=100_000)] = ""`

### Service layer extensions

- [ ] **FR-028** — `TaxomeshService.update_category(category_id, name, description) -> Category`
- [ ] **FR-029** — `TaxomeshService.update_item(item_id, name, description, enabled) -> Item`
- [ ] **FR-030** — `TaxomeshService.update_tag(tag_id, name) -> Tag`
- [ ] **FR-031** — `TaxomeshService.delete_tag(tag_id) -> None`
- [ ] **FR-032** — `TaxomeshService.place_item_in_category(item_id, category_id, sort_index=0) -> ItemParentLink`; idempotent

### Repository layer extensions

- [ ] **FR-033** — `TaxomeshRepositoryBase.delete_tag(tag_id: UUID) -> bool` (16th method)
- [ ] **FR-034** — `TaxomeshRepositoryBase.save_item_parent_link(link: ItemParentLink) -> None` (17th); upsert on `(item_id, category_id)`
- [ ] **FR-035** — `TaxomeshRepositoryBase.list_item_parent_links() -> list[ItemParentLink]` (18th)
- [ ] **FR-036** — `JsonRepository` implements `delete_tag`, `save_item_parent_link`, `list_item_parent_links`

### README

- [ ] **FR-037** — README updated: CLI section appears before Python API quick start

### Tests

- [ ] **FR-038** — `tests/test_cli.py` covers: all sub-commands (happy path), all not-found error paths, config file loading, default-config fallback, inline `--category-id`/`--tag-id` on `item add`, `item add-to-category`, `item add-to-tag`, `category add --parent-id`; uses Typer's `CliRunner`
...
- [ ] **SC-003** — `taxomesh item add --external-id 99 --name "Song"` exits 0, prints item
- [ ] **SC-004** — `taxomesh item add --external-id 1 --name "X" --category-id <id>` places item in category
